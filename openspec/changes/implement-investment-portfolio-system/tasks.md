...
- [ ] 1.2 Configurar propriedades compartilhadas, perfis `dev` e `test`, UTC, timeout de clientes e variáveis de ambiente.
...
- [ ] 4.1 Configurar clientes OpenFeign, DTOs de integração e conversão centralizada de erros HTTP externos.
- [ ] 4.2 Implementar adapters de cotação para os provedores brasileiro e americano e o contrato comum de consulta.
- [ ] 4.3 Implementar facades de CNPJ, CEP e CVM, com normalização dos dados de corretora.
- [ ] 4.4 Implementar serviço e endpoints de corretora, recusando persistência sem todas as validações obrigatórias.
- [ ] 4.5 Adicionar testes com dublês para sucesso, não encontrado, autenticação, rate limit, timeout e indisponibilidade das integrações.
...
- [ ] 6.1 Implementar criação imutável de operação e cálculo de valor bruto, líquido e valores opcionais.
- [ ] 6.2 Implementar compra com preço informado e compra dependente de cotação, criando ou incrementando a posição na mesma transação.
- [ ] 6.3 Implementar venda dependente de cotação, validação de saldo de ações e manutenção de posição zerada.
- [ ] 6.4 Expor endpoints de compra, venda, busca e histórico paginado sem endpoints de edição de operação.
- [ ] 6.5 Testar rollback em falhas, venda superior à posição, venda total, ordenação do histórico e concorrência otimista.
